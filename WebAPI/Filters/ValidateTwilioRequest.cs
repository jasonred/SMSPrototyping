...
namespace WebAPI.Filters
{
    ...
    public class ValidateTwilioRequestImprovedAttribute : ActionFilterAttribute
    {
        ...
        {
            _isTestEnvironment = bool.Parse(ConfigurationManager.AppSettings["Twilio:IsTestEnvironment"]);
            _authToken = ConfigurationManager.AppSettings["Twilio:AuthToken"];
            _urlSchemeAndDomain = ConfigurationManager.AppSettings["Twilio:WebHookBaseUrl"];
        }

        public override async Task OnActionExecutingAsync(HttpActionContext actionContext, CancellationToken cancellationToken)
        {
            if (!await IsValidRequestAsync(actionContext.Request) && !_isTestEnvironment)
            {
